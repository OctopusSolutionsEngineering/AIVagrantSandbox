...
# thing on both sides is the path relative to that root. That is what gets sent; the
...
$codeRoot = if ($env:AGENT_CODE_DIR) { $env:AGENT_CODE_DIR } else { Join-Path $HOME 'Code' }
...
elseif ($startNorm.StartsWith("$rootNorm/", $comparison)) {
     ...
     [Console]::Error.WriteLine("qwen.ps1: $startDir is outside $codeRoot, which is the only directory " +
                                'synced into the guest — starting the agent at the root of the synced tree')
     $startRel = '.'
}
...
# the directory holding this script. Taken from $PSScriptRoot rather than written out, so
...
try {
    ...
    if (-not (Test-Path -LiteralPath 'Vagrantfile')) {
        throw "no Vagrantfile in $PSScriptRoot — this script has to stay beside it"
    }
}
catch {
...
vagrant ssh -c "exec sudo /usr/local/sbin/qwen-agent --dir $startRelQ" -- -t -R 64342:127.0.0.1:64342 -R 11434:127.0.0.1:11434 -L 127.0.0.1:7777:127.0.0.1:7777 -R 127.0.0.1:10000:127.0.0.1:10000 -R 127.0.0.1:10001:127.0.0.1:10001 -R 127.0.0.1:10002:127.0.0.1:10002
exit $LASTEXITCODE